...
#define NUM_THREADS (2)     // service threads
...
/** 
 * @brief Semaphores used as synchronization primitives
 *
 * The semaphores semT1, semT2 and semT3 belong to the three individual threads and their execution
 * functions. These semaphores will ensure that these functions are not interrupted and that
 * concurrency bugs do not appear when they are into the critical section of their execution.
 *
 * The semaphore semG is a semaphore protecting the access to the global critical variable gsum that
 * stores the summation of all the thread summations. Protection of this global variable is essential
 * as without a semaphore, the main process thread can read corrupted data.
 *
 */
 
sem_t semT1, semT2, semT3, semG;
 ...
 *
 * The functions here define the threads that the main process thread will execute using a user-
 * specified scheduling policy.
 *
 * @param *threadp which represents the thread index.
 * @return  void. 
 */
...
/** 
 * @brief Scheduling Policy Function
 * 
 * Prints the scheduling policy that is implemented.
 *
 * @param void
 * @return  void 
 */
...
/** 
 * @brief Summation Functions (sum_1, sum_2 and sum_3)
 * 
 * Functions used by the three threads for summing up the 
 * integers between 1 and 299.
 *
 * @param void
 * @return  void 
 */
 ...
 * @brief Main Function
 * 
 * Main process thread that follows the user-specified scheduling policy and executes the three threads
 * previously declared. It sums up the integer values between 1 and 299 and then prints the total
 * that is stored in a global variable with the use of concurrency and synchronization primitives by the
 * three threads.
 *
...
{   
	...
	// Arguments check for the command line entry point
	// If the value matches, the system will follow that scheduling policy
	
	if(argc < 2)
	{
		//printf("Please specify the scheduling policy...\n");
		exit(-1);
	}
	else
	{
		if(!strcmp(argv[1], "SCHED_FIFO"))
		{
			sched_policy = 1;
		}
		...
		else if(!strcmp(argv[1], "SCHED_OTHER"))
		{
			sched_policy = 0;
		}
		else
		{
			//printf("Pthread Policy is UNKNOWN\n"); 
			//printf("Correct usage: SCHED_FIFO or SCHED_RR or SCHED_OTHER.\n");
			exit(-1);
		}
	}
    ...
    //
    if (sem_init (&semT1, 0, 0)) { printf ("Failed to initialize semT1 semaphore\n"); exit (-1); }
    if (sem_init (&semT2, 0, 0)) { printf ("Failed to initialize semT2 semaphore\n"); exit (-1); }
    if (sem_init (&semT2, 0, 0)) { printf ("Failed to initialize semT2 semaphore\n"); exit (-1); }
    if (sem_init (&semG, 0, 0)) { printf ("Failed to initialize semT2 semaphore\n"); exit (-1); }
	
	// Releasing the semaphores for the threads
	...
	// Getting the process ID and then setting the user-specified scheduling policy
	...
	//Printing the scheduling policy
	print_scheduler();
	...
	//Setting up the scheduling policy, parameters and attributes for the threads
    ...
    for(i=0; i < NUM_THREADS; i++)
    {
		...
		rc=pthread_attr_setschedpolicy(&rt_sched_attr[i], sched_policy);
		rc=pthread_attr_setaffinity_np(&rt_sched_attr[i], sizeof(cpu_set_t), &threadcpu);

		rt_param[i].sched_priority=rt_max_prio - prio;
    ...
    rc=pthread_create(&threads[0],               // pointer to thread descriptor
                      ...
                      thread_1,                     // thread function entry point
    ...
    rc=pthread_create(&threads[1],               // pointer to thread descriptor
                      ...
                      //(void *)0,                 // default attributes
                      thread_2,                     // thread function entry point
...
/*                     
    ...
    rc=pthread_create(&threads[2],               // pointer to thread descriptor
                      ...
                      //(void *)0,                 // default attributes
                      thread_3,                     // thread function entry point
...
*/
   ...
   for(i=0;i<NUM_THREADS;i++)
	...
	// Displaying the total summation value with the safe of concurrency
