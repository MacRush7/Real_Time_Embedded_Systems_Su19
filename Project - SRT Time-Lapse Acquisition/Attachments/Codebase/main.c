/******************************************************************************************************/
/* ECEN 5623 RTES Summer 2019 - Time-Lapse Final Project Code										  */
/*                                                                          						  */
/* Student Name: Rushi James Macwan, August 17, 2019                         						  */
/******************************************************************************************************/
...
/*																									  */
/* The original code owned by Dr. Siewert can be found by visiting the below link:					  */
/* Link [1]: http://ecee.colorado.edu/~ecen5623/ecen/ex/Linux/code/sequencer_generic/seqgen.c         */
/*																									  */
/* Other resource Link: 																			  */
/* https://stackoverflow.com/questions/20332382/linux-sleeping-with-clock-nanosleep/20332855		  */
/*																									  */
/******************************************************************************************************/
 ...
 *
 * @file main.c 
 * @brief With the help of a RT scheduling policy (e.g. SCHED_FIFO and RMA algorithm), 3 SRT services are
 * run on CPU core 3 and 1 BE service is run on CPU core 2. The services enable the system to communicate
 * with a camera device (C270 HD Camera) for frame acquisition and the same is used to allow the system to
 * come up as a time-lapse project.
 * 
...
#define TEST_FRAMES 	(9)
#define FREQ			(1)
#define TEST_SECONDS	(181)

/* The system is set to test the camera at 1Hz for 181
 * seconds. For 10 Hz, the FREQ #define statement can
 * be set at 10. Also, the TEST_FRAMES may be increased
 * to 200 with the seconds kept the same for a testing
 * time of 3 minutes and 1 second.
 *
 * For 1 Hz, ideal TEST_FRAMES = 9
 *           FREQ = 1
 *           TEST_SECONDS = 181
 *
 * For 10 Hz, ideal TEST_FRAMES = 199
 *			  FREQ = 10
 *			  TEST_SECONDS = 181
 *
 * Please Note: The TEST_FRAMES begin from 0 and
 * therefore please use 9 frames for 1 Hz and 199 frames
 * for 10 Hz stable testing.
 *
 * To exploit the system limitations, comment out the
 * TEST_SECONDS and TEST_FRAMES from the code and
 * update the SequencePeriods and mcnt variables
 * appropriately (5700 SequencePeriods for 1 Hz for 3 minutes 1 second and
 * 6030 SequencePeriods for 10 Hz with mcnt to be 9 and 200 in the two cases.
 */
...
#define NUM_THREADS (3+1) // SRT 3 services (including the Sequencer) and 1 BE service

///////////////////////////////////////////////////////////////////////////////////////////////////////
// GLOBAL VARIABLES
///////////////////////////////////////////////////////////////////////////////////////////////////////

// Flag for protecting memory corruption and to prevent system loading from the use of a semaphore
...
char 					ppm_header[]="P6\n#9999999999 sec 9999999999 msec \n"HRES_STR" "VRES_STR"\n255\n AUTHOR: RUSHI JAMES MACWAN\n Device Name:- RPI3B+\n ECEN 5623 RTES Su-19\n";
char 					ppm_dumpname[]="test00000000.ppm";
char 					pgm_header[]="P5\n#9999999999 sec 9999999999 msec \n"HRES_STR" "VRES_STR"\n255\n AUTHOR: RUSHI JAMES MACWAN\n Device Name:- RPI3B+\n ECEN 5623 RTES Su-19\n";
char 					pgm_dumpname[]="test00000000.pgm";
unsigned int 			framecnt=0; // Frame counter global variable

// Circular buffers for storing the image frames

unsigned char 			bigbuffer[500][(1280*960)];
unsigned int			bigwcnt=0;
unsigned int			bigrcnt=0;

int						size_cb[500];
unsigned int			frmcnt_cb[500];
struct 		timespec 	frm_tim_cb[500];
...
void main(int argc, char** argv)
{
    ...
    threadParams[0].sequencePeriods = (unsigned long long) ((((unsigned long long) TEST_FRAMES) + ((unsigned long long)(((unsigned long long) (TEST_SECONDS)) * FREQ))) * ((unsigned long long)(30/FREQ)));
...
// Flag to keep the BE service running until other services are not exited
int flag=0;
...
void *Sequencer(void *threadp)
{
    ...
    do
    {
        ...
        if((seqCnt % ((int)(30/FREQ))) == 0) sem_post(&semS1);

        // Service_2 = RT_MAX-2	@ 1 Hz
        if((seqCnt % ((int)(30/FREQ))) == 0) sem_post(&semS2);
...
void *Service_2(void *threadp)
{
    ...
    while(!abortS2)
    {
		...
		if(mcnt > TEST_FRAMES)
		{
			...
			if(!mflag)
			{
				dump_pgm(bigbuffer[bigrcnt], size_cb[bigrcnt], frmcnt_cb[bigrcnt], &frm_tim_cb[bigrcnt]);
				bigrcnt++;
			}
		}
...
static void dump_pgm(const void *p, int size, unsigned int tag, struct timespec *time)
{
	...
	syslog(LOG_CRIT, "frame: %08d Time: %010d sec %010d msec", tag, (int)time->tv_sec, (int)((time->tv_nsec)/1000000));
...
static void process_image(const void *p, int size)
{
	...
	for(i=0, newi=0; i<size; i=i+4, newi=newi+6)
	{
		...
		yuv2rgb(y_temp, u_temp, v_temp, &bigbuffer[bigwcnt][newi], &bigbuffer[bigwcnt][newi+1], &bigbuffer[bigwcnt][newi+2]);
		yuv2rgb(y2_temp, u_temp, v_temp, &bigbuffer[bigwcnt][newi+3], &bigbuffer[bigwcnt][newi+4], &bigbuffer[bigwcnt][newi+5]);
	}
	
#ifdef DUMP1

	dump_ppm(bigbuffer[bigwcnt], ((size*6)/4), framecnt, &frame_time);
	...
	for(i=0, newi=0; i<size; i=i+4, newi=newi+2)
	{
		...
		bigbuffer[bigwcnt][newi]=pptr[i];
		bigbuffer[bigwcnt][newi+1]=pptr[i+2];
		
		
	}
	
	size_cb[bigwcnt] = size;
	frmcnt_cb[bigwcnt] = framecnt;
	frm_tim_cb[bigwcnt] = frame_time;
	bigwcnt++;
	
	mflag = 0;
#ifdef DUMP2
	//dump_pgm(bigbuffer[bigwcnt], (size/2), framecnt, &frame_time);
#endif
